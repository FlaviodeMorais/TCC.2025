...
export class EmulatorService {
  ...
  private generateDefaultReading(): ThingspeakResponse {
    return {
      ...
      field2: (this.config.sensorRanges.waterLevel.current / 100).toString(), // Convertido para decimal (0-1)
  ...
  private async generateReading(): Promise<void> {
    ...
    try {
      ...
      switch (this.config.mode) {
        case 'stable':
          ...
          reading = {
            ...reading,
            field1: this.config.sensorRanges.waterTemp.current.toString(),
            field2: (this.config.sensorRanges.waterLevel.current / 100).toString(),
            field3: this.config.controlStates.pumpStatus ? '1' : '0',
            field4: this.config.controlStates.heaterStatus ? '1' : '0',
            field7: this.config.sensorRanges.flowRate.current.toString(),
            field8: this.config.sensorRanges.humidity.current.toString()
        ...
        case 'fluctuating':
          ...
          reading = {
            ...
            field2: (waterLevel / 100).toString(), // Converter para decimal (0-1)
            field3: this.config.controlStates.pumpStatus ? '1' : '0',
            field4: this.config.controlStates.heaterStatus ? '1' : '0',
      ...
      const insertReading: InsertReading = {
        ...
        level: parseFloat(String(reading.field2 || '0')) * 100, // Convertido para porcentagem (0-100)
        pumpStatus: reading.field3 === '1',
        heaterStatus: reading.field4 === '1',
        timestamp: new Date()
      };
